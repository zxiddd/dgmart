...
import {
  MapPin, Package, Star, TrendingUp, Wifi, WifiOff,
  ChevronRight, Clock, Bike, RefreshCw,
} from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const handleToggleOnline = async () => {
    ...
    try {
      ...
      const newStatus = res.data?.is_online;
  ...
  const handleAcceptOrder = async (order) => {
    ...
    try {
      ...
      const msg = err.response?.data?.message || 'Could not accept order';
  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      {!isVerified && (
        <div className="mx-5 mb-3 px-4 py-3 bg-amber-50 border border-amber-200 rounded-2xl flex items-start gap-3">
          <span className="text-lg mt-0.5">⏳</span>
          <div>
            <p className="text-sm font-bold text-amber-800">Verification Pending</p>
            <p className="text-xs text-amber-600 mt-0.5">
              Our team is reviewing your documents. You can explore the app meanwhile.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-3 mx-5 mb-5">
        ...
        <StatCard
          label="Rating"
          value={parseFloat(todayStats.rating || 5).toFixed(1)}
          icon={<Star className="w-4 h-4" />}
          color="yellow"
        />
      </div>

      {/* Active Order Pinned Banner */}
      {activeOrder && (
        <div className="mx-5 mb-5">
          ...
          <div className="space-y-3 pb-4">
            {availableOrders.map((order) => (
              <AvailableOrderCard
                ...
                loading={claimingId === order.order_id}
              />
            ))}
          </div>
        )}
  ...
  return (
    <button
      ...
      className={clsx(
        ...
        isOnline
          ...
          : 'bg-gradient-to-r from-gray-700 to-gray-800 shadow-lg shadow-gray-200',
    ...
    >
      <div className="flex items-center gap-4">
        ...
        <div className="relative">
          <div className={clsx(
            ...
            {isOnline ? (
              ...
              <WifiOff className="w-6 h-6 text-white/70" />
            )}
          </div>
          {isOnline && (
            <span className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-white rounded-full border-2 border-green-500 animate-pulse-slow" />
          )}
        </div>
        <div className="text-left">
          ...
          <p className={clsx('text-xs font-medium mt-0.5', isOnline ? 'text-green-100' : 'text-gray-400')}>
            {isOnline ? 'Receiving order requests' : 'Tap to go online & earn'}
          </p>
        </div>
      </div>
      ...
      <div className={clsx(
        ...
        isOnline ? 'bg-white/30 justify-end' : 'bg-white/10 justify-start'
      )}>
...
function StatCard({ label, value, icon, color }) {
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-2xl p-3 shadow-sm">
      <div className={clsx('w-8 h-8 rounded-xl flex items-center justify-center mb-2', colorMap[color])}>
        {icon}
      </div>
...
function ActiveOrderBanner({ order, onContinue }) {
  return (
    <button
      onClick={onContinue}
      className="w-full bg-gradient-to-r from-primary-500 to-orange-600 rounded-2xl p-4 text-left shadow-lg shadow-orange-200 active:scale-[0.98] transition-transform"
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-orange-100 text-xs mt-0.5">
              {order.customer_name || 'Customer'} · #{order.order_number || ''}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-white/80 text-xs font-bold px-2 py-1 bg-white/20 rounded-lg">
      ...
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="text-right">
          ...
          <p className="text-base font-extrabold text-green-600">
            ₹{order.delivery_fee || order.earning || '40'}
          </p>
        </div>
      </div>
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1 text-gray-400">
            <span className="text-xs font-semibold text-gray-600">
              ₹{order.total || order.order_total || '—'} order
            </span>
          </div>
          {order.estimated_time && (
            <div className="flex items-center gap-1 text-gray-400">
              <Clock className="w-3 h-3" />
              <span className="text-xs">{order.estimated_time} min</span>
            </div>
          )}
        </div>
        <button
          ...
          disabled={loading}
          className={clsx(
            'px-5 py-2.5 rounded-xl text-sm font-bold text-white transition-all active:scale-95',
            loading
              ? 'bg-gray-300 cursor-not-allowed'
              : 'bg-primary-500 shadow-md shadow-orange-200 hover:bg-primary-600'
          )}
        >
          {loading ? '...' : 'ACCEPT →'}
        </button>
  ...
  return (
    <div className="flex flex-col items-center justify-center py-14 text-center">
      ...
      <button
        onClick={onGoOnline}
        disabled={disabled}
        className="px-8 py-3.5 bg-green-500 text-white font-bold rounded-2xl shadow-lg shadow-green-200 active:scale-95 transition-transform disabled:opacity-50"
      >
        GO ONLINE 🟢
      </button>
    </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center py-14 text-center">
      ...
      <p className="text-sm text-gray-400 max-w-[240px]">
        Waiting for orders... New requests will appear here automatically.
      </p>
    </div>
  );
}
